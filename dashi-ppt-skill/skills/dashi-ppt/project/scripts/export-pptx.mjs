...
//
// Usage:
//   node scripts/export-pptx.mjs <deck-ppt-dir> [out-file.pptx] [--pdf] [--title <text>] [--selected-only]
...
// --selected-only exports only each logical page's selected variant (N slides
// instead of 4N), honoring variantOutputMode:"selected-only" in the deck.
...
async function main() {
  ...
  try {
    ...
    const exportOptions = {
      outFile,
      title,
      ...(selectedOnly ? { variantOutputMode: 'selected-only' } : {}),
    };
    if (pdfMode) {
      const result = await exportScreenshotPdfFromUrl(browser, url, exportOptions);
      console.log(`PDF exported: ${path.relative(CALLER_CWD, outFile)} (${result.pages} page(s))`);
    } else {
      const result = await exportEditablePptxFromUrl(browser, url, exportOptions);
...
function printUsage() {
  console.error('Usage:');
  console.error('  node scripts/export-pptx.mjs <deck-ppt-dir> [out-file.pptx] [--pdf] [--title <text>] [--selected-only]');
